...
namespace FreyrSenseCollector.Collectors
{
    ...
    public class SenseApiCollector
    {
        ...
        public SenseApiCollector(ILogger logger)
        {
            _logger = logger;
        }
    ...
    //}
        ...
        private string GetGenericJsonToString(SenseApiSupport senseApiSupport,string url)
        {
            ...
            {
                Log.Add($"Failed prettifying the json string from {url}", e);
                return json;
            }            
        }
        ...
        private Guid GetGuid(JToken funky, string[] names)
        {
            ...
            {
                Log.Add($"Failed Getting Guid {names?.FirstOrDefault()}", e);
                return Guid.Empty;
            }
        ...
        private int GetInt(JToken funky, string name)
        {
            ...
            {
                Log.Add($"Failed Getting int {name}", e);
                return -1;
            }
        }
        private DateTime GetDate(JToken funky, string name)
        {
            ...
            {
                Log.Add($"Failed Getting Date {name}", e);
        ...
        private string GetString(JToken funky, string[] names)
        {
            ...
            {
                Log.Add($"Failed Getting Strings {names?.FirstOrDefault()}", e);
                return string.Empty;
            }
        }
        private string GetString(JToken funky, string name)
        {
            ...
            {
                Log.Add($"Failed Getting String {name}", e);
                return string.Empty;
            }
        }
        ...
        private bool GetBool(JToken funky ,string name)
        {
            ...
            {
               Log.Add($"Failed Getting bool {name}",e);
                return false;
            }
        }
